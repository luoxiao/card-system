<?php
header('Content-type: text/html; charset=utf-8'); require_once 'model/builder/AlipayTradeRefundContentBuilder.php'; require_once 'service/AlipayTradeService.php'; if (!empty($_POST['out_trade_no']) && trim($_POST['out_trade_no']) != '') { $sp2714cc = trim($_POST['out_trade_no']); $spb91753 = trim($_POST['refund_amount']); $sp56802e = trim($_POST['out_request_no']); $sp114e2c = ''; $sp929e8e = new AlipayTradeRefundContentBuilder(); $sp929e8e->setOutTradeNo($sp2714cc); $sp929e8e->setRefundAmount($spb91753); $sp929e8e->setOutRequestNo($sp56802e); $sp929e8e->setAppAuthToken($sp114e2c); $sp2b1698 = new AlipayTradeService($sp4d5cc2); $sp642597 = $sp2b1698->refund($sp929e8e); switch ($sp642597->getTradeStatus()) { case 'SUCCESS': echo '支付宝退款成功:' . '<br>--------------------------<br>'; print_r($sp642597->getResponse()); break; case 'FAILED': echo '支付宝退款失败!!!' . '<br>--------------------------<br>'; if (!empty($sp642597->getResponse())) { print_r($sp642597->getResponse()); } break; case 'UNKNOWN': echo '系统异常，订单状态未知!!!' . '<br>--------------------------<br>'; if (!empty($sp642597->getResponse())) { print_r($sp642597->getResponse()); } break; default: echo '不支持的交易状态，交易返回异常!!!'; break; } return; } ?>
